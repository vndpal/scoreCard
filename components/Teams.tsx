import { View, Text, StyleSheet, ScrollView, Pressable, Modal, FlatList, TextInput, TouchableOpacity, Alert } from "react-native";
...
const Teams = () => {
    ...
    const [selectedTeamInitials, setSelectedTeamInitials] = useState<string | null>(null);
    ...
    const filteredTeams = teams.filter(team => {
        ...
        const playerMatch = players.some(p => p.name.toLowerCase().includes(query));
    ...
    useEffect(() => {
        if (mainSearchQuery) {
            ...
            filteredTeams.forEach(team => {
                const players = teamPlayersMap[team.teamInitials] || [];
                const playerMatch = players.some(p => p.name.toLowerCase().includes(mainSearchQuery.toLowerCase()));
                if (playerMatch) {
    ...
    useEffect(() => {
        const fetchTeamsAndPlayers = async () => {
            ...
            try {
                ...
                const matches = await Match.getAllOrderby(club.id, "startDateTime", "desc");
                ...
                console.log("Mappings:", playersMap);
    ...
    const getAvailablePlayers = () => {
        ...
        return allPlayers.filter(p => !assignedPlayerIds.has(p.id) && p.name.toLowerCase().includes(searchQuery.toLowerCase()));
    };
    ...
    const handleAddPlayerToTeam = async (playerToAdd: player) => {
        ...
        try {
            ...
            // Optimistic update
            const newTeamPlayersMap = { ...teamPlayersMap, [selectedTeamInitials]: updatedPlayers };
            setTeamPlayersMap(newTeamPlayersMap);

            await TeamPlayerMapping.createOrUpdate(selectedTeamInitials, club.id, updatedPlayers);
    ...
    const handleRemovePlayerFromTeam = async (teamInitials: string, playerId: string) => {
        try {
            const currentPlayers = teamPlayersMap[teamInitials] || [];
            const updatedPlayers = currentPlayers.filter(p => p.id !== playerId);

            // Optimistic update
            const newTeamPlayersMap = { ...teamPlayersMap, [teamInitials]: updatedPlayers };
            setTeamPlayersMap(newTeamPlayersMap);

            await TeamPlayerMapping.createOrUpdate(teamInitials, club.id, updatedPlayers);
        } catch (error) {
            ...
            <View style={styles.modalOverlay}>
                <View style={[styles.modalContent, themeStyles.card]}>
                    <View style={styles.modalHeader}>
                        <Text style={[styles.modalTitle, themeStyles.text]}>Add Player to {selectedTeamInitials}</Text>
                        <IconButton icon="close" size={24} onPress={() => setIsAddPlayerModalVisible(false)} iconColor={currentTheme === "dark" ? "#FFF" : "#000"} />
                    </View>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <TouchableOpacity style={[styles.modalPlayerItem, { borderBottomColor: currentTheme === "dark" ? "#333" : "#E9ECEF" }]} onPress={() => handleAddPlayerToTeam(item)}>
                                <View style={[styles.playerAvatar, { backgroundColor: currentTheme === "dark" ? "#00C4B4" : "#8E24AA", marginRight: 12 }]}>
                                    <Text style={styles.playerAvatarText}>{item.name.charAt(0).toUpperCase()}</Text>
                                </View>
                                <Text style={[styles.modalPlayerName, themeStyles.text]}>{item.name}</Text>
                                <Ionicons name="add-circle-outline" size={24} color={currentTheme === "dark" ? "#00C4B4" : "#8E24AA"} />
                            </TouchableOpacity>
                        )}
                        ListEmptyComponent={() => (
                            <Text style={[styles.emptyStateText, themeStyles.subText]}>No available players found</Text>
                        )}
                    />
                </View>
            </View>
                                ...
                                >
                                    <View style={styles.cardHeader}>
                                        <View style={styles.teamInfo}>
                                            ...
                                            <View style={styles.teamDetails}>
                                                ...
                                                <Text style={[styles.teamInitials, themeStyles.subText]}>
                                                    {team.teamInitials} • {players.length} player{players.length !== 1 ? 's' : ''}
                                                </Text>
                                            </View>
                                        </View>
                                    ...
                                    {isExpanded && (
                                        <View style={styles.playersSection}>
                                            {players.length === 0 ? (
                                                <Text style={[styles.noPlayersText, themeStyles.subText]}>
                                                        ...
                                                        <View
                                                            ...
                                                            style={[
                                                                styles.playerItem,
                                                                {
                                                                    ...
                                                                    borderBottomColor: currentTheme === "dark" ? "#333" : "#E9ECEF",
                                                                },
                                                            ]}
                                                        >
                                                            ...
                                                            <View style={styles.playerRight}>
                                                                <Text style={[styles.playerNumber, themeStyles.subText]}>
                                                                    #{index + 1}
                                                                </Text>
                                                                {!isLiveMatchOngoing && (
                                                                    <IconButton
                                                                        ...
                                                                        size={20}
                                                                        onPress={() => handleRemovePlayerFromTeam(team.teamInitials, player.id)}
                                                                        iconColor={currentTheme === "dark" ? "#FF6B6B" : "#D32F2F"}
                                                                    />
                                                                )}
                                                            </View>
                                                        </View>
                                            ...
                                            <Button
                                                ...
                                                onPress={() => {
                                                    if (isLiveMatchOngoing) {
                                                        Alert.alert("Action Restricted", "Cannot add players while a match is ongoing.");
                                                        return;
                                                    }
                                                    handleOpenAddPlayerModal(team.teamInitials);
                                                }}
                                                disabled={isLiveMatchOngoing}
                                                style={{ marginTop: 12, borderColor: isLiveMatchOngoing ? "#666" : (currentTheme === 'dark' ? '#00C4B4' : '#8E24AA'), opacity: isLiveMatchOngoing ? 0.5 : 1 }}
                                                textColor={isLiveMatchOngoing ? "#666" : (currentTheme === 'dark' ? '#00C4B4' : '#8E24AA')}
                                            >
                                                {isLiveMatchOngoing ? "Match in Progress" : "Add Player"}
                                            </Button>
                                        </View>
                                    )}
                                </Pressable>
