...
- **Added columns are auto-backfilled — no manual full refresh needed.** Before each table loads, the script compares its code schema against the live BigQuery table's columns (`schemaExtended` / `getTableColumns`, a free metadata call). If the code defines a column the table doesn't have yet, that one table does a one-time full Firestore re-read so *every* existing row gets the new column's real value (instead of `NULL` for the untouched docs an incremental run would miss). Only the changed table pays this cost, once; unaffected tables stay incremental, and subsequent runs go back to incremental for the changed table too. So adding a field to a schema just requires a normal run. **Caveat:** this is name-based — it detects *added* columns only. **Type changes** (e.g. INTEGER→FLOAT) and **renames** are not detected and still need `FULL_REFRESH=true`.
- A `FULL_REFRESH=true` env flag is kept as an escape hatch for the cases auto-detection doesn't cover — a column type change or rename, or recovering from a bad run:
  ```bash
  # Windows PowerShell
